...
class FedTwinCRLoss(CrossEntropyLoss):
    ...
    def forward(self, input_p, input_g, target, rounds, epoch, args, noise_prior=None):
        ...
        ind_noise_p = torch.ByteTensor([])
        ind_noise_g = torch.ByteTensor([])
        if rounds <= args.begin_sel:  # 如果在前30epoch集中式，对应联邦应该是30/local_epoch
            ...
            ind_noise_p = torch.ByteTensor([not x for x in ind_p_update])
            ind_noise_g = torch.ByteTensor([not x for x in ind_g_update])
        ...
        return loss_p, loss_g, len(loss_batch_p), len(loss_batch_g), ind_noise_p, ind_noise_g
