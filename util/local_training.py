...
class FedTwinLocalUpdate:
    ...
    def update_weights(self, net_p, net_glob, rounds):
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels, indexes) in enumerate(self.ldr_train):
                ...
                for _ in range(self.args.K):
                    ...
                    loss_p, loss_g, len_loss_p, len_loss_g, ind_noise_p, ind_noise_g = self.loss_func(log_probs_p, log_probs_g,
                                                                                   labels, rounds, iter, self.args)

                    if rounds == self.args.rounds1 - 1 and iter == self.args.local_ep - 1:
                        ind_noise_p = indexes[ind_noise_p]
                        for index in ind_noise_p:
                            #print(index)
                            m = 0
                            for n, indexx in enumerate(indexes):
                                if indexx == index:
                                    m = n
                                    break
                            noise_index_output[index] = log_probs_p[m].detach()
        ...
        return net_p.state_dict(), net_glob.state_dict(), sum(epoch_loss) / len(epoch_loss), n_bar_k, noise_index_output
