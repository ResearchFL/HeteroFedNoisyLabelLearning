    ...
    if filename.endswith('.log'):
        ...
        if match:
            ...
            with open(basePath+filename, 'r') as file:
                ...
                # else:
                #     print("not find")
                values = np.array([re.findall(r'Round {} global test acc  (\d+\.\d+)'.format(Rounds[dataset_idx]-i-1), contents) for i in range(1)])
                values = values.astype(np.float64).squeeze()
                if values.ndim > 1:
                    values = np.mean(values.astype(np.float64), axis=0)
                if values.any():
