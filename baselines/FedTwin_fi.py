...
import copy
from util.aggregation import personalized_aggregation, FedAvg
import numpy as np
from util.load_data import load_data_with_noisy_label
# from data.dataloader_getter_utils import *
from util.local_training import FedTwinLocalUpdate, FedAVGLocalUpdate, globaltest, personalizedtest
import time
...
def FedTwin_fi(args):
    ...
    copy_noise_index_output_all = noise_index_output_all
    path = args.dataset + args.level_n_system + args.iid
    np.save(path + '.npy', copy_noise_index_output_all)
    torch.save(netglob.state_dict(), path + ".pth")
