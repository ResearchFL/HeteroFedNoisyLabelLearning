...
import numpy as np
from util.local_training import GlobalCORESUpdate, globaltest
import copy
from util.aggregation import FedAvg


def GlobalCORES(args):
    ...
    for rnd in range(args.rounds2):
        ...
        for idx in idxs_users:  # training over the subset
            local = GlobalCORESUpdate(args=args, dataset=dataset_train, idxs=dict_users[idx])
            w_local, loss_local = local.update_weights(net=copy.deepcopy(model).to(args.device), rounds=rnd)
